...
'''
InvDN的推理需要正向传播一次加逆向传播一次，为了适配动转静，需要将两次传播合入到一次forward中。
...
def get_args(add_help=True):
    ...
    parser = argparse.ArgumentParser(
        description="INVDN DENOISING", add_help=add_help)

    parser.add_argument(
        '--clean-dir',
        type=str,
        default="./SIDD_mini/val_mini/GT/0_0.PNG",
        help='path to clean data')
    parser.add_argument(
        '--noisy-dir',
        type=str,
        default="./SIDD_mini/val_mini/Noisy/0_0.PNG",
...
class InferenceEngine(object):
    ...
    def preprocess(self, clean_dir, noisy_dir):
        ...
        """
        clean = load_img(clean_dir)
        noisy = load_img(noisy_dir)

        clean = clean.transpose([2, 0, 1])
        noisy = noisy.transpose([2, 0, 1])

        clean = np.expand_dims(clean, 0) #1 3 256 256
        noisy = np.expand_dims(noisy, 0)

        # 动转静模型运行时不支持random，需要在输入时引入随机噪声。
        random_noise = np.random.randn(1, 45, noisy.shape[2], noisy.shape[3]).astype(np.float32)
        noisy = np.concatenate([noisy, random_noise], 1)
