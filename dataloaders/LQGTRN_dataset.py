...
class LQGTRNDataset(Dataset):
    ...
    def __init__(self, opt, is_train):
        ...
        self.data_type = "img"
        # if opt.get('dataroot_GT', None) is not None: #TODO
        #     opt['dataroot_GT'] = osp.expanduser(opt['dataroot_GT'])
        # if opt.get('dataroot_LQ', None) is not None:
        #     opt['dataroot_LQ'] = osp.expanduser(opt['dataroot_LQ'])
        if self.is_train:
    ...
    def __getitem__(self, index):
        ...
        if self.is_train:
            ...
            H, W, _ = img_GT.shape
            if H < GT_size or W < GT_size: #一般不经过
            ...
            img_LQ, img_GT, img_Noisy = util.augment([img_LQ, img_GT, img_Noisy], True, True) #TODO 增强要不要当超惨
