...
def main():
    ...
    model_dir = os.path.join(opt.TRAINING.SAVE_DIR, mode, 'models-channel-4p', session)
    log_dir = os.path.join(opt.TRAINING.SAVE_DIR, mode, 'logs-channel-4p', session)
    ...
    scheduler = optim.lr.MultiStepDecay(learning_rate=new_lr, milestones=opt.OPTIM.MILESTONES, gamma=opt.OPTIM.GAMMA) #TODO 应该是这个
    wd_G = opt.OPTIM.WEIGHT_DECAY_G if opt.OPTIM.WEIGHT_DECAY_G else 0
    clip_grad_norm = nn.ClipGradByNorm(opt.OPTIM.GRADIENT_CLIPPING) #应该是这样，原本在l_total.backward()和optimizer.step()之间
    ...
    with LogWriter(logdir=log_dir) as writer:
        ...
        while current_iter <= total_iters:
            ...
            for data in train_loader:
                ...
                x_samples_image = x_samples[:, :3, :, :] #没必要
                ...
                if current_iter % eval_now == 0 and local_rank == 0:
                    ...
                    with paddle.no_grad():
                        ...
                        for data_val in val_loader:
                            ...
                            # fake_H = paddle.clip(fake_H, 0, 1) #TODO 应该要
