...
def forward_x8(x, forward_function, gaussian_scale):
    def _transform(v, op):
        v2np = v.cpu().numpy()
        if op == 'v':
            tfnp = v2np[:, :, :, ::-1].copy()
    ...
    back_list = []
    for data in lr_list:
        y_forw = paddle.concat((data[:, :3, :, :], gaussian_scale * gaussian_batch(data[:, 3:, :, :].shape)), axis=1)
...
def Multi_forward_x8(x, forward_function, gaussian_scale=1, sample_num=16):
    def _transform(v, op):
        v2np = v.cpu().numpy()
        if op == 'v':
            tfnp = v2np[:, :, :, ::-1].copy()
    ...
    for data in lr_list:
        ...
        for i in range(sample_num):
            y_forw = paddle.concat((data[:, :3, :, :], gaussian_scale * gaussian_batch(data[:, 3:, :, :].shape)), axis=1)
            fake_Hs.append(forward_function(x=y_forw, rev=True)[:, :3, :, :])
        fake_H = paddle.stack(fake_Hs, axis=0)
        fake_H = fake_H.mean(axis=0)
